...
function generateApiCrud(modelName: string, fields: Array<{ name: string; type: string }>): void {
  ...
  const controllerContent = generateControllerContent(modelName, fields);
  ...
  const serviceContent = generateServiceContent(modelName, fields);
 ...
 */
function generateControllerContent(modelName: string, fields: Array<{ name: string; type: string }>): string {
 ...
 */
function generateServiceContent(modelName: string, fields: Array<{ name: string; type: string }>): string {
...
function generateFormComponentContent(modelName: string, fields: Array<{ name: string; type: string }>): string {
  ...
  const initialFormData = fields.map(field => {
    ...
    if (field.type === 'Date') return `    ${field.name}: new Date(),`;
    return `    ${field.name}: '',`;
  }).join('\n');
